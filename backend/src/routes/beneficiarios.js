...
const { validateBody, crearBeneficiarioSchema, editarBeneficiarioSchema } = require('../utils/validators');
...
router.get('/buscar', authMiddleware, asyncHandler(async (req, res) => {
  const { q = '' } = req.query;
  if (q.length < 2) {
...
router.post('/', authMiddleware, validateBody(crearBeneficiarioSchema), asyncHandler(async (req, res) => {
...
router.put('/:id', authMiddleware, validateBody(editarBeneficiarioSchema), asyncHandler(async (req, res) => {
  const { ruc_cedula, nombre } = req.body;
